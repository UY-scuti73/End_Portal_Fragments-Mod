...
public class PortalHelper {
    ...
    public static void tryPortal(Level level, BlockPos pos) {
        ...
        // Reduces the blocks to only the ones needed
        ...
        // Checks to make sure all portal frames are what they are meant to be
        ArrayList<String> types = new ArrayList<>(Arrays.asList(
                "ruined", "golden", "ancient", "dark",
                "withered", "gusty", "wealthy", "sandy"
        ));
        for (BlockPos blockPos : allFrames) {
            BlockState state = level.getBlockState(blockPos);
            if (!(state.getBlock() instanceof AbstractEndPortal)) {
                return;
            } else if (!state.getValue(EYE)) {
                return;
            } else {
        ...
        for (BlockPos blockPos : portalPos) {
            level.setBlock(blockPos, Blocks.END_PORTAL.defaultBlockState(), Block.UPDATE_ALL);
        }
