...
public class PortalHelper {
    ...
    public static void tryPortal(World world, BlockPos pos) {
        //Gets an 8x8 area around where the eye was placed
        ...
        //Get the normal end frames to establish a perimeter
        ArrayList<BlockPos> normalFrames = new ArrayList<>();
        for (BlockPos blockPos : blockArea) {
            if (world.getBlockState(blockPos).getBlock() == Blocks.END_PORTAL_FRAME) {normalFrames.add(blockPos);}
        }

        //Return if there aren't 4 normal end portal frames in the area
        if (normalFrames.size() < 4) {return;}

        //Gets the max and mins for sides
        ...
        //Reduces the blocks to be only the ones needed
        ...
        //Checks to make sure all portals are what they are meant to be
        ArrayList<String> types = new ArrayList<>(Arrays.asList("ruined", "golden", "ancient", "dark", "withered", "gusty", "wealthy", "sandy"));
        for (BlockPos blockPos : allFrames) {
            BlockState state = world.getBlockState(blockPos);
            if (!(state.getBlock() instanceof AbstractEndPortal)) {
                return;
            } else if (!state.get(EYE)) {
                return;
            } else {
        ...
        for (BlockPos blockPos : normalFrames) {
            ...
            if (!state.get(EYE)) {return;}
        }

        if (!types.isEmpty()) {return;}

        //Sets up the portals if all requirements have been met
